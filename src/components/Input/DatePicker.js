...
const DatePicker = ({ value, onChange, minDate, onClose, ...otherProps }) => {
  ...
  const handlerKeyDown = (event) => {
    ...
    if (keyCode === KeyCodeUtils.ENTER) {
      event.preventDefault();
      const valueMoment = moment(event.target.value, "DD/MM/YYYY");
      onChange([valueMoment.toDate(), valueMoment.toDate()]);
    }
  };
  ...
  const autoFormatOnChange = (value, seperator) => {
    ...
    let values = input.split(seperator).map((v) => v.replace(/\D/g, ""));

    if (values[0]) values[0] = checkDateValue(values[0], 31);
    if (values[1]) values[1] = checkDateValue(values[1], 12);
    let output = values.map((v, i) =>
      v.length === 2 && i < 2 ? v + " " + seperator + " " : v
    );
    return output.join("").substr(0, 14);
  };

  const onInputChange = (e) => {
    let converted = autoFormatOnChange(e.target.value, "/");
    e.target.value = converted;
  };

  useEffect(() => {
    const node = flatpickrNode.current.node;
